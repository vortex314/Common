...
#define __FLE__ strrchr(__FILE__, '/') ? strrchr(__FILE__, '/') + 1 : __FILE__
#define INFO(fmt, ...) \
  if (logger.enabled(Log::LOG_INFO)) LOGF(fmt, ##__VA_ARGS__)
#define ERROR(fmt, ...) \
  if (logger.enabled(Log::LOG_ERROR)) LOGF(fmt, ##__VA_ARGS__)
#define WARN(fmt, ...) \
  if (logger.enabled(Log::LOG_WARN)) LOGF(fmt, ##__VA_ARGS__)
#define FATAL(fmt, ...) \
  if (logger.enabled(Log::LOG_FATAL)) LOGF(fmt, ##__VA_ARGS__)
#define DEBUG(fmt, ...) \
  if (logger.enabled(Log::LOG_DEBUG)) LOGF(fmt, ##__VA_ARGS__)
#define TRACE(fmt, ...) \
  if (logger.enabled(Log::LOG_TRACE)) LOGF(fmt, ##__VA_ARGS__)
