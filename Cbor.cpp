...
Cbor::Cbor(uint8_t* data, uint32_t length) : Bytes(data, length) {}
...
bool Cbor::get(bool& bl) {
  CborVariant v;
  PackType type;
  if (readToken(type, v) != E_OK) return false;
  if (type == P_BOOL) {
    bl = v._bool;
    return true;
  } else if (type == P_NILL) {
...
bool Cbor::get(Bytes& bytes) {
  ...
  if (readToken(type, v) == E_OK && (type == P_BYTES || type == P_STRING)) {
    bytes.clear();
    for (uint32_t i = 0; i < v._uint64; i++) bytes.write(read());  // skip data
    return true;
  } else if (type == P_NILL) {
    return true;
  }
...
bool Cbor::get(Str& str) {
  ...
  if (readToken(type, v) == E_OK && type == P_STRING) {
    str.clear();
    for (uint32_t i = 0; i < v._uint64; i++) str.write(read());  // skip data
    return true;
  } else if (type == P_NILL) {
    return true;
  }
...
Erc Cbor::peekToken(PackType& type) {
  if (!hasData()) {
    WARN("no data");
    return ENODATA;
  };
  ...
  if (minor < 24) {
    ...
    // suppose very big length will be stopped by BREAK, side effect limited
    // arrays and maps can also be breaked
  }
  if (type == P_SPECIAL) switch (minor) {
      case 21: {  // TRUE
        type = P_BOOL;
...
Erc Cbor::readToken(PackType& type, CborVariant& v) {
  ...
  if (minor < 24) {
    ...
    value = 65535;  // suppose very big length will be stopped by BREAK, side
                    // effect limited arrays and maps can also be breaked
  }
  if (type == P_SPECIAL) switch (minor) {
      case 21: {  // TRUE
        type = P_BOOL;
...
Cbor::PackType Cbor::tokenToString(Str& str) {
  CborVariant v;
  PackType type;
  if (readToken(type, v) != E_OK) return P_ERROR;
  switch (type) {
    case P_PINT: {
    ...
    case P_MAP: {
      ...
      for (int i = 0; i < count; i++) {
        if (i) str << ",";
        if (tokenToString(str) == P_BREAK) break;
        str << ":";
        tokenToString(str);
      }
      str << "}";
...
Erc Cbor::toString(Str& str) {
  ...
  while (hasData()) {
    ...
    if (ct == P_BREAK || ct == P_ERROR) return E_INVAL;
    if (hasData()) str << ",";
  };
  return E_OK;
}

Cbor& Cbor::operator<<(int i) { return add(i); }
...
Cbor& Cbor::add(Str& str) {
  ...
  str.offset(0);
  while (str.hasData()) write(str.read());
...
Cbor& Cbor::operator<<(const char* s) { return add(s); }
...
Cbor& Cbor::operator<<(uint64_t ull) { return add(ull); }
...
Cbor& Cbor::addKey(uint32_t key) { return add(key); }
