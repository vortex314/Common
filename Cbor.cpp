...
bool Cbor::get(std::string& str) {
    CborVariant v;
    PackType type;
    if (readToken(type, v) == E_OK && type == P_STRING) {
        str.clear();
        for (uint32_t i = 0; i < v._uint64; i++)
            str.push_back(read()); // skip data
        return true;
    } else if (type == P_NILL) {
        return true;
    }
...
Cbor& Cbor::add(std::string& str) {
    addToken(P_STRING, str.size());
...
bool Cbor::vaddf(const char* fmt, va_list args) {
    ...
    while (*fmt != '\0') {
        //		LOGF("%c",*fmt);
        if (*fmt == '2') {
            uint16_t i = va_arg(args, uint32_t);
            add(i);
        } else if (*fmt == 'i') {
...
bool Cbor::vscanf(const char* fmt, va_list args) {
    while (*fmt != '\0') {
        if (*fmt == '2') {
            uint16_t* pi = va_arg(args, uint16_t*);
            if (get(*pi) == false)
                return false;
        } else if (*fmt == 'i') {
